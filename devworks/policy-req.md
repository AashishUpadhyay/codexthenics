...
**Auditing the effective merged policy.** Run `/permissions` in a Claude Code session: it lists every active permission rule together with the settings file each one came from, showing the effective merged result across managed, CLI, local, repo, and global layers in one place. Without a running session, read `.claude/settings.local.json`, `.claude/settings.json`, and `~/.claude/settings.json` together — these are the persistent file layers, not the full precedence chain from above, since managed (enterprise) policy and CLI flags aren't files you can read offline — and apply deny-then-ask-then-allow across all three combined, not per file — a deny anywhere in that set wins regardless of which file it's in.
